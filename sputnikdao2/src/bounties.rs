...
impl Contract {
    ...
    /// 현상금을 지급하는 제안이 성공적으로 표결에 부쳐졌거나 그렇지 않을 때 이 제안을 해야 합니다.
...
impl Contract {
    ...
    // 주어진 예상 실행 시간으로 호출자에 의해 주어진 바운티를 주장합니다.
    /// Bond must be attached to the claim.
    // 클레임에는 보증금이 첨부되어야 합니다.
    /// Fails if already claimed `times` times.
    // 이미 'times' 번 주장한 경우 실패합니다
    #[payable]
    ...
    /// 이 바운티에서 'claimer_id'의 claim 을 제거합니다.
