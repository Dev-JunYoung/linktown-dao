...
// 이 계약을 배포한 팩토리에 관한 정보와 자동 업데이트가 허용되는지 여부에 대한 정보.
...
// 스토리지에서 팩토리 정보를 가져옵니다.
// 설계상 팩토리에서 멈춘 계약의 업그레이드를 허용하기 위해 계약 STATE를 사용하지 않습니다.
...
///  - current account, in case of fetching contract code from factory;
///  - factory, if this contract allows to factory-update;
// 새로운 계약을 받아 업데이트하고 마이그레이션을 호출하는 함수.
// 호출할 수 있는 두 가지 옵션이 있습니다:
//  - 팩토리에서 계약 코드를 가져올 경우 현재 계정;
//  - 이 계약이 팩토리 업데이트를 허용하는 경우 팩토리;
#[no_mangle]
pub fn update() {
